...
def main(page: ft.Page):  
                ...
                page.controls=[  
                    ...
                    [
                        ft.Row([
                            ft.Column([
                                ft.Text("Input"),
                                ft.Column(controls=inp),                   
                            ],spacing=10,width=150),

                            ft.Column([
                                ft.Text("Output"),
                                ft.Column(controls=outp),                   
    ...
    def items(count):
        ...
        for i in range(0, count):
            items.append(
                ft.TextField(label="Byte "+str(i),value="0",height=36, width=120,on_submit=lambda e,index=i: text_submitted(e,f"{index}"))
            )
        return items
    ...
    page.navigation_bar = ft.NavigationBar(
        on_change=_on_press,
        destinations=[
            ft.NavigationDestination(icon=ft.icons.EXPLORE, label="Scan"),
            ft.NavigationDestination(icon=ft.icons.COMMUTE, label="Slave"),
